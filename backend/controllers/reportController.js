...
//   try {
//     const { id } = req.params;
//     const { radiusKm = 200 } = req.query; // Default 50km radius
...
//---------------------------------------- NEW CODE AFTER NEAR SOCIAL MEDIA FIX ----------------------------------------
const Report = require("../models/ReportSchema");
const SocialMedia = require("../models/socialMediaSchema");
const { validationResult } = require("express-validator");
const axios = require("axios");
...
exports.getReportWithNearbySocialMedia = async (req, res) => {
  try {
    const { id } = req.params;
    const { radiusKm = 50 } = req.query; // Default 200km radius
    ...
    const nearbySocialMedia = await SocialMedia.aggregate([
      ...
      {
        $sort: { postTimestamp: -1 }, // Changed from 'date' to 'postTimestamp'
      },
      {
        $limit: 20,
      },
