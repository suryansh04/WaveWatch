...
const ViewMap = () => {
  ...
  const createHotspotLayer = (hotspot) => {
    ...
    // In createHotspotLayer function, update the social media section:
    if (hotspot.type === "social") {
      popupContent = `
    <div class="view-map-hotspot-popup">
      <h3>Social Media Hotspot</h3>
      ...
      <div class="view-map-hotspot-report-types">
        ...
        ${[...new Set(hotspot.posts.map((p) => p.eventType || "Unknown"))].join(
          ", "
        )}
        <br><strong>Platforms:</strong><br>
        ${[
          ...new Set(hotspot.posts.map((p) => p.platform || "Social Media")),
        ].join(", ")}
      </div>
    </div>
  `;
  ...
  const getFilteredSocialMediaPosts = () => {
    ...
    // Filter based on advanced filters if socialMediaTypes are selected
    let filtered = socialMediaPosts.filter(
      (post) => post.latitude && post.longitude
    );

    // Apply event type filter if specified
    if (advancedFilters.socialMediaTypes.length > 0) {
      filtered = filtered.filter((post) =>
        advancedFilters.socialMediaTypes.includes(post.eventType)
      );
    }
  ...
  useEffect(() => {
    ...
    <div class="view-map-social-media-popup">
      <h3>Social Media Alert</h3>
      <p><strong>Source:</strong> ${post.platform || "Social Media"}</p>
      <p><strong>Event Type:</strong> ${post.eventType || "Unknown"}</p>
      <p><strong>Content:</strong> ${
        post.text ? post.text.substring(0, 100) + "..." : "No text available"
      }</p>
      <p><strong>Date:</strong> ${new Date(
        post.postTimestamp
      ).toLocaleString()}</p>
      <p><strong>Engagement:</strong> ${post.likes || 0} likes, ${
        post.comments || 0
      } comments</p>
      <button class="view-map-detail-btn" onclick="window.openSocialDetail('${
        post._id
        ...
        <div className="view-map-controls-panel">
          ...
          <div className="view-map-panel-card view-map-advanced-filters-card">
            ...
            <div className="view-map-filters-content">
              ...
              {/* Threats */}
              {/* Social Media Events - Add this after the Threats section */}
              <div className="view-map-filter-group">
                <label className="view-map-filter-label">
                  <AlertTriangle size={14} className="view-map-label-icon" />{" "}
                  Social Media Events
                ...
                <div className="view-map-checkbox-grid">
                  {[
                    ...
                    <label key={type} className="view-map-checkbox-pill">
                      <input
                        type="checkbox"
                        checked={tempFilters.socialMediaTypes.includes(type)}
                        onChange={() =>
                          handleCheckboxFilterChange("socialMediaTypes", type)
                        }
                      />
                      <span>
